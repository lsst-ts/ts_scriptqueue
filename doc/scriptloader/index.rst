...
* `BaseScript` base class for Python scripts.
* `ScriptLoader`: a `Commandable SAL Component`_ (CSC) to load SAL scripts.
  Note: a script queue is likely to replace this CSC.
* `LoaderModel`: a class that loads and configures SAL scripts and keeps track of loaded scripts.
